    ...
    try {
        ...
        if (email.toLowerCase() === ADMIN_EMAIL.toLowerCase() && password === ADMIN_PASSWORD) {
            ...
            return res.json({
                ...
                token,
                bearer_token: token,
                user: { id: 'admin-001', email: ADMIN_EMAIL, name: 'Admin', role: 'admin', status: 'active', premium: true }
            });
        }
...
app.all('/api/auth/validation/*', async (req, res) => {
    ...
    try {
        ...
        const { data: user } = await supabase
            .from('users')
            .select('id, email, name, role, status, plan, expires_at')
            .eq('id', decoded.id)
            .maybeSingle();
        ...
        res.json({
            success: true,
            status: isActive ? 'authorized' : 'expired',
...
app.get('/api/admin/users', authenticateAdmin, async (req, res) => {
    const { data, error } = await supabase
        .from('users')
        .select('id, email, name, role, status, plan, expires_at, created_at')
...
app.post('/api/admin/login', (req, res) => {
    ...
    // Aceitar qualquer admin@... para compatibilidade
