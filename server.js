...
const nitroEntry = path.join(__dirname, 'frontend', '.output', 'server', 'index.mjs');

if (fs.existsSync(nitroEntry)) {
    console.log('🚀 Ligando o motor Backs ZapCRM (Nitro)...');
    import(nitroEntry);
} else {
    console.error('❌ ERRO CRÍTICO: Motor do site não encontrado em frontend/.output/server/index.mjs');
